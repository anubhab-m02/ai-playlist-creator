...
import { Loader2, Music, Image as ImageIcon, PlusCircle, Save, Trash2, ChevronLeft, Sparkles, LogIn } from 'lucide-react'; // Removed Briefcase
import { collection, addDoc, doc, updateDoc, serverTimestamp } from 'firebase/firestore';

const PlaylistCreator = ({ existingPlaylist, onSaveSuccess }) => {
    const { db, currentUser, appId } = useFirebase(); // Removed spotifyAccessToken, spotifyProfile
    ...
    // Removed: const [isSavingToSpotify, setIsSavingToSpotify] = useState(false);
    ...
    const callGeminiAPI = async (prompt, responseSchema) => { /* ... same as before ... */ 
        if (!geminiApiKey) { throw new Error("Gemini API key is not configured."); }
        const payload = { contents: [{ role: "user", parts: [{ text: prompt }] }], generationConfig: responseSchema ? { responseMimeType: "application/json", responseSchema } : {} };
        const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${geminiApiKey}`;
        const response = await fetch(apiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        if (!response.ok) { const errData = await response.json(); console.error("Gemini API error:", errData); throw new Error(errData?.error?.message || `API Error ${response.status}`); }
    ...
    const handleGetSongIdeas = async () => { /* ... same as before ... */ 
    ...
    const handleSuggestPlaylistTitle = async () => { /* ... same as before ... */ 
    ...
    const handleGenerateLinerNotes = async () => { /* ... same as before ... */ 
        const currentThemeForPrompt = originalThemePrompt.trim() || theme.trim();
    ...
    const addSongToPlaylist = (song) => { /* ... same as before ... */ 
    ...
    const handleSavePlaylistToFirestore = async () => {
        if (!currentUser || !db) { displayError("Cannot save: User not authenticated or DB not ready."); return false; }
        if (!theme.trim()) { displayError("Playlist title/theme cannot be empty."); return false; }
        if (songs.length === 0) { displayError("Add at least one song."); return false; }
        setIsSaving(true);
        const playlistData = { 
        ...
        try {
            ...
            if (existingPlaylist?.id) {
                ...
                await addDoc(collection(db, path), playlistData);
            ...
            return true;
        } catch (err) { console.error("Save to Firestore error:", err); displayError(`Save to Firestore failed: ${err.message}`); return false; }
        finally { setIsSaving(false); }
    };

    // Removed: handleSaveToSpotify function
    ...
    if (!currentUser) { /* ... same as before ... */ 
        return (<div className="text-center py-10 text-gray-400 flex-grow flex flex-col items-center justify-center"><LogIn size={48} className="mx-auto mb-4" /><h2 className="text-2xl mb-2">Please sign in</h2><p>You need to be signed in to create a mixtape.</p></div>);
    }
    ...
    return (
        <div className="max-w-4xl mx-auto bg-slate-800/70 p-6 md:p-8 rounded-xl shadow-2xl space-y-8 backdrop-blur-sm">
            ...
            {/* Section 1: Name Your Mixtape (Theme input, AI Title Suggestion, Get Song Ideas button) */}
            <div className="space-y-4 p-6 bg-slate-700/50 rounded-lg shadow-inner">
                {/* ... UI for theme input, AI title suggestion, get song ideas ... (same as before) ... */}
            ...
            {/* Section 2 & 3: Gemini's Booth & Your Mixtape (Song suggestions, current playlist) */}
            {(aiSuggestions.length > 0 || songs.length > 0 || isLoadingSuggestions) && (
                <div className="grid md:grid-cols-2 gap-6">
                    {/* ... UI for Gemini's Booth and Your Mixtape ... (same as before) ... */}
                    <div className="p-6 bg-slate-700/50 rounded-lg shadow-inner">
                        ...
                        {!isLoadingSuggestions && aiSuggestions.length === 0 && songs.length > 0 && <p className="text-sm text-gray-400 italic">No new AI suggestions.</p>}
            ...
            {/* Section 4 & 5: Cover Art (URL input only) & Liner Notes */}
            {(theme || songs.length > 0) && (
                <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div className="p-6 bg-slate-700/50 rounded-lg shadow-inner space-y-3">
                        {/* ... UI for Liner notes ... (same as before) ... */}
                        <label htmlFor="linerNotes" className="block text-xl font-medium text-purple-300">5. Add Liner Notes</label>
                        <p className="text-sm text-gray-400 mb-2">Share the story or mood behind your mixtape.</p><textarea id="linerNotes" value={linerNotes} onChange={(e) => setLinerNotes(e.target.value)} rows="5" placeholder="e.g., 'A collection of tracks for late-night drives...'" className="w-full p-3 bg-slate-800 border border-slate-600 rounded-md focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none placeholder-gray-500 custom-scrollbar"></textarea><button type="button" onClick={handleGenerateLinerNotes} disabled={isLoadingLinerNotes || songs.length === 0 || (!originalThemePrompt.trim() && !theme.trim())} className="w-full sm:w-auto flex items-center justify-center px-4 py-2 bg-teal-500 hover:bg-teal-600 text-white font-semibold rounded-md shadow-md transition-all duration-200 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-teal-400 focus:ring-opacity-50 text-sm">{isLoadingLinerNotes ? <Loader2 className="animate-spin mr-2" size={18}/> : <Sparkles className="mr-2" size={18} />} Write Liner Notes</button>
                    </div>
                </div>
            )}

            {/* Save Button (only "Save to Maestro" now) */}
            <div className="pt-6 border-t border-slate-700 flex flex-col sm:flex-row gap-4">
                ...
                {/* Removed "Save to Spotify" button */}
            </div>
             <style jsx global>{`
