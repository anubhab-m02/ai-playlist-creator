...
import { Loader2, Music, Sparkles, Tag, X, Disc3, Plus, Eye, EyeOff } from 'lucide-react';
            ...
            <div>
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                    <input 
                        ...
                        id="theme" 
                        value={theme} 
                        onChange={handleThemeInputChange} // Directly call the prop
                    ...
                    <button 
                        type="button" 
                        onClick={handleSuggestPlaylistTitle} // Directly call the prop
            ...
            <div>
                ...
                {tags.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                        {tags.map(tag => (
                            <span key={tag} className="flex items-center bg-purple-600 text-white text-xs font-semibold px-3 py-1 rounded-full">
                                {tag.charAt(0).toUpperCase() + tag.slice(1)}
                                <button type="button" onClick={() => handleRemoveTag(tag)} className="ml-2 text-purple-200 hover:text-white">
                                    <X size={14}/>
                                </button>
                            </span>
                        ))}
                ...
                {seedSongInputs.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                        {seedSongInputs.map((seed, index) => (
                            <span key={index} className="flex items-center bg-emerald-600 text-white text-xs font-semibold px-3 py-1 rounded-full">
                                <Disc3 size={14} className="mr-1.5"/> {seed}
                                <button type="button" onClick={() => handleRemoveSeedSong(seed)} className="ml-2 text-emerald-200 hover:text-white">
                                    <X size={14}/>
                                </button>
                            </span>
                        ))}
